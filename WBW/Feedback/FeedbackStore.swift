...
final class FeedbackStore: ObservableObject {
    ...
    func submit(_ draft: FeedbackDraft, token: String) async -> APIClient.FeedbackSubmitOutcome {
        ...
        do {
            ...
            // retry ไม่ได้แล้ว (400/401/500 ฯลฯ) — คืน .failed ตรงๆ ห้ามแตะคิว: draft ของ attempt นี้เอง
            // ไม่เคยถูก add (add เกิดเฉพาะตอน AppError.offline ด้านบน) ส่วนของที่อาจค้างอยู่ก่อนหน้าเป็น
