...
extension Color {
    ...
    /// แดงที่ขอบจอสถานะ SOS ระหว่างเคสยังเปิดอยู่ (ดู `SOSEmergencyBackdrop`)
    ///
    /// **คงที่ทั้งสองธีมเหมือน `wbwForestVoid`** — จอนี้มืดเสมอไม่ว่าผู้ใช้เลือกธีมไหน และสิ่งที่
    /// สีนี้บอก ("เครื่องนี้กำลังรอความช่วยเหลือ") ไม่ควรแปรตามค่าตั้งรูปลักษณ์ของใคร
    /// · เข้มกว่า `wbwDanger` หนึ่งขั้นโดยตั้งใจ: มันเรืองอยู่หลังตัวอักษรตลอดเวลา ไม่ใช่ป้ายเตือน
    /// ที่โผล่มาแวบเดียว สว่างกว่านี้แล้วข้อความใกล้ขอบจออ่านไม่ออก
    static let wbwEmergencyEdge = hex(0x8C1D18)
