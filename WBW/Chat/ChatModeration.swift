...
enum ChatModeration {
    ...
    /// ส่งข้อความนี้ขึ้นกลุ่มได้ไหม — **ด่านเดียวของทั้งแอป ห้ามเรียก `ChatWordFilter` ตรง ๆ**
    ///
    /// Guideline 1.2 บังคับให้มีตัวกรองก่อนโพสต์ (ดูเหตุผลเต็มที่ `ChatWordFilter`) · ให้จอ
    /// เรียกที่นี่ที่เดียวด้วยเหตุผลเดียวกับ `ChatDraft.canSend`: ตัวตัดสินสองตัวที่ไม่ตรงกัน
    /// แปลว่าปุ่มเปิดให้กดกับข้อความที่ปลายทางจะเตะทิ้ง แล้วข้อความหายเงียบโดยไม่มีอะไรบอก
    static func allowsSending(_ text: String) -> Bool { !ChatWordFilter.isObjectionable(text) }
