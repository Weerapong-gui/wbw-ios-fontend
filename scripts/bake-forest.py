...
# tree_near ลดจาก 60 เหลือ 34 พร้อมกับดันแถบออก (ดูคอมเมนต์ที่ scatter() เรียก tree_near ใน
# build_forest — Task 8b) แถบแคบลง (23→16 หน่วย) ยิ่งใช้ 60 ต้นเดิมจะยิ่งอัดแน่นกว่าเดิม ไม่ใช่โปร่งขึ้น
COUNTS = {"tree_near": 34, "tree_far": 30, "grass": 450, "rock_small": 25, "rock_large": 10}
...
def build_forest():
    ...
    stats = {}
    # Task 8b: เดิมแถบต้นไม้ใกล้เริ่มที่ y=3 — เกือบจ่อกล้อง (อยู่ (0,0,1.7)) ลำต้น/ทรงพุ่มเลยเต็มจอ
    # ไม่มีระยะกลางให้ตา ภาพเลยแบน (ยืนยันด้วย render-check.py ก่อนแก้: เห็นแต่ลำต้นชนเลนส์) ดันจุด
    # เริ่มออกไปที่ y=10 (พ้นขอบวงเคลียริ่งที่ (0,6) รัศมี 3.2 ด้วย คือ 6+3.2=9.2) เปิดทางเดินโล่ง
    # ด้านหน้าให้เห็น ปลายอีกด้านยังชนกับจุดเริ่มของ tree_far (26.0) เหมือนเดิม ไม่เปิดช่องว่างกลาง
    # ผลพลอยได้: ต้นไม้ที่โตตามเช็คอินซึ่งอยู่ที่ y=6 (ดู GrowingTree.position) พ้นแถบต้นไม้หนาทึบนี้ไป
    # เลย ไม่มีอะไรมาบังจนกลืนกับพุ่มไม้รอบข้างอีก (ปัญหาที่ task-6-report.md บันทึกไว้ว่าแก้จากฝั่ง
    # Swift ไม่ได้)
    stats["tree_near"] = scatter(rand, sources["tree"], COUNTS["tree_near"],
                                 (10.0, 26.0), 4.0, (0.65, 1.45), 1.6, cache, coll)
    # แถบไกลคงที่เดิม — brief สั่งให้แตะเฉพาะแถบใกล้
    ...
    # Task 8b: ทดสอบด้วย render-check.py หลังดันเฉพาะ tree_near แล้วพบว่าหิน rock_large ก้อนใหญ่
    # ยังหลุดมาวางใกล้กล้องพอจะทำให้ภาพแบนซ้ำแบบเดียวกับต้นไม้เดิม — ก้อนหินไม่ได้อยู่ใน "แถบต้นไม้" ที่
    # brief พูดถึงตรงๆ แต่ปัญหาเดียวกัน (วัตถุใหญ่ชนกล้อง) ยืนยันตัวการด้วย raycast จากตำแหน่งกล้องจริง
    # ไม่ใช่เดาจากภาพ (สูง 0.9-2.0 หน่วยตาม HEIGHTS ก่อน scatter-scale — สูงเกือบเท่าความสูงกล้องเอง 1.7
    # ที่ระยะ ~8-9 หน่วยเลยยังกินจอเยอะ) ลองดันเป็น 8.0 ก่อนแล้วยังไม่พอ (ยืนยันด้วย raycast ซ้ำ) จึงดันไป
    # 14.0 แทน — ไม่ลดขนาด/สัดส่วน (ของเดิมอิงเว็บ) แค่ย้ายให้ไกลจากเลนส์เหมือนที่ทำกับต้นไม้ ปลายอีกด้าน
    # (30.0) กับจำนวน (10) คงเดิม
    stats["rock_large"] = scatter(rand, sources["rock_large"], COUNTS["rock_large"],
                                  (14.0, 30.0), 4.0, (0.6, 1.4), 0.9, cache, coll)
