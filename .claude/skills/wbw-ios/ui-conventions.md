...
## กล้องสแกนของเจ้าหน้าที่ — หยุดแล้วต้องมีคนสั่งเริ่ม

`ScannerVC` ใน `WBW/StaffScanView.swift` หยุด session ที่ `viewWillDisappear` และ**ต้องมี
`viewWillAppear` สั่งเริ่มคู่กันเสมอ** — เคยมีแต่ทางหยุด เจ้าหน้าที่สลับไปแท็บ SOS แล้วกลับมา
(หรือโดนจอเคสใหม่ทับ ซึ่งเป็นสถานการณ์ที่ออกแบบไว้ตั้งใจ) ได้ช่องมองดำถาวรจนกว่าจะปิดแอป

ช่องทางเดียวที่ SwiftUI สั่งกล้องได้คือ `updateUIViewController` — ปล่อยว่างเมื่อไหร่ ปุ่ม
เปิด/ปิดกลายเป็นปุ่มที่กดแล้วไม่มีอะไรเกิดขึ้นทันที · ตรรกะการรับสแกนอยู่ที่
`WBW/StaffScanLogic.swift` ไม่ใช่ใน View **เพราะซิมูเลเตอร์ไม่มีกล้อง** เส้นทางการสแกนจึงไม่มี
ทางถูกทดสอบเลยถ้าฝังไว้ในจอ

`WBWTests/ScannerLifecycleTests.swift` ค้ำสี่อย่าง: มีทางเริ่มไม่ใช่มีแต่ทางหยุด ·
`updateUIViewController` ยังส่งคำสั่งลงไปจริง · `rectOfInterest` ถูกตั้ง (ไม่งั้นกรอบเล็ง
เป็นของประดับ คนต่อคิวข้างหลังถูกเช็คอินแทนได้) · มีตัวรับตอนระบบยึดกล้องไป (Split View บน iPad)
