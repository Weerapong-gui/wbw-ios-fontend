...
*(ขั้นตอนถ่ายจอ SU RUN ระหว่างเดินจริงถูกถอดออกจากที่นี่ — `SURunView` เข้าไม่ถึงในแอปแล้ว
ดูหัวข้อถัดไป · วิธีป้อนพิกัดด้วย `xcrun simctl location <dev> start` ยังอยู่ที่
`.claude/skills/wbw-ios/build-and-run.md` เผื่อวันที่จอนั้นถูกต่อกลับเข้ามา)*
...
**สองจอในชุดเดิมหายไปเพราะเข้าไม่ถึงในแอปแล้ว** — `SURunView` (จับระยะเดิน/นับก้าว) และ
`MyQRCodeView` (QR เต็มจอ) ไม่มีโค้ดไหนอ้างถึงเลยทั้งแอป มีแค่ `#Preview` ของตัวเอง
โค้ดยังอยู่ครบและมีเทสคุม แต่ผู้ใช้กดเข้าไปไม่ได้ · ที่ว่างสองใบแทนด้วย `05-activities`
