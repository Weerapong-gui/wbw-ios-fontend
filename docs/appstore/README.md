...
**ชุดล่าสุดถ่ายใหม่ทั้ง 10 ใบเมื่อ 2026-08-24** (ก่อน archive 1.0 (12)) — ชุด 2026-08-22
ใช้ไม่ได้แล้วเพราะแอปขยับสามที่หลังจากนั้น: แท็บแผนที่เปิดมาที่ 2 มิติแทน 3 มิติ (`02-map`),
ปุ่ม SOS มีบรรทัดเตือนว่าไม่ใช่หน่วยกู้ชีพเพิ่มมา (`10-sos`) และการ์ดให้ความเห็นเคยเป็น
แผ่นขาวที่ตัวอักษรหายไปกับพื้นในโหมดมืด ซึ่งแก้แล้วในคอมมิตเดียวกับที่ถ่ายชุดนี้ (`08-feedback`)

ชุดก่อนหน้าถ่าย 2026-08-22 — **ชุดเดิมกว่านั้น (2026-08-19) ใช้ไม่ได้แล้ว** แอปถูกออกแบบใหม่
...
| `02-map` | แผนที่ 2 มิติ (ภาพดาวเทียม + เส้นทาง + หมุดฐาน) + การ์ดฐานเปิดอยู่ | `-uitestTab 1 -uitestMapPin 5` |
...
**ชุดที่อัปขึ้น ASC ตอนนี้ยังเป็นภาพแผนที่ 3 มิติ — ต้องถ่าย `02-map` ใหม่ก่อนส่งรอบหน้า**
ค่าเริ่มต้นของแท็บแผนที่เปลี่ยนเป็น 2 มิติเมื่อ 2026-08-24 (`MapMode.stored`) รูปเดิมจึงไม่ตรงกับ
สิ่งที่ผู้ตรวจเปิดมาเจอ ซึ่งเป็นเหตุ 2.3.3 ตรง ๆ · คำสั่งถ่ายไม่เปลี่ยน เพราะค่าเริ่มต้นเปลี่ยนให้แล้ว

**`02-map` รอ tile ดาวเทียมโหลด ไม่ใช่รอโมเดลอีกแล้ว** — ต้องมีเน็ตตอนถ่าย และต้องเปิดรูปดูว่า
ภาพพื้นมาครบไม่ใช่ตารางเทา ๆ (MapKit วาดตารางว่างระหว่างรอ tile โดยไม่มี error ให้เห็น) ·
ถ้าจะถ่ายโหมด 3 มิติ ใส่ `-uitestMapMode 3d` แล้วต้องรอโมเดล `map.usdz` 9.4 MB (entity 2,201 ก้อน)
13 วิได้ตัวหมุน "กำลังโหลดแผนที่" แทนโมเดล · 6.5 ใช้ 40 วิพอ ส่วน 6.9 ต้องถึง 90 วิ
