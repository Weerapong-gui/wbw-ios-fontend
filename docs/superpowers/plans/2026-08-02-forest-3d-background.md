...
- **`xcodebuild build` alone will happily serve a stale `.usdz` out of DerivedData and still print `** BUILD SUCCEEDED **`.** Measured on this machine: after `forest.usdz` was re-baked and committed, a plain build ran zero compile and zero resource-copy steps and the app launched with the *previous* asset, reproducing a bug that had already been fixed. Any time you are verifying appearance, or any time an asset under `WBW/Resources/` has changed, build with `clean` or a fresh `-derivedDataPath`. A screenshot taken after a plain build is not evidence.
