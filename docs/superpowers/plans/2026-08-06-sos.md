...
>    `clearForLogout()` exactly as before. Known, accepted residual risk: `SOSOutbox` is namespaced
>    only by backend, not by user, so a *different* account logging in on the same device before the
>    original owner re-authenticates could inherit the draft — judged acceptable because an automatic
>    401 is, in practice, overwhelmingly the same person's token expiring, not a device handoff.
...
>    longer recognizes at all (`activeCall` returning `nil` every time, which also covers whatever
>    `try?` swallows from a thrown error, since `activeSOS` doesn't distinguish the two) cannot poll
>    forever; the counter resets on any non-nil response.
>
...
> **What actually shipped, across two review rounds — see `task-14-report.md` for both.**
>
