...
Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/SOSOutboxTests 2>&1 | tail -20`
...
> **Post-review addition (from Task 14, not part of this task's original delivery):** added
> `var isActive: Bool` to `SOSStatus` — `true` for `.queued`/`.received`/`.onTheWay`, `false` for
> `.closed`. Task 14's button needs to know whether a case is already open before it may call
> `SOSStore.raise()` again (that method has no re-entrancy guard of its own — see the post-review
> note at the end of Task 12's section), and `.draft == nil` turned out to be the wrong check: a
> resolved case leaves `draft` non-nil forever (see the `startStatusPoll` note in Task 12's section),
> which would have permanently blocked a second SOS after the first one ever completed. Purely
> additive, no existing case renamed or removed. See Task 14's report for the full reasoning.

> **Second post-review addition (from Task 14's third review round):** added `let ownerId: String`
> to `SOSDraft`, no default value — every construction site must be explicit. `SOSOutbox` is keyed
> only by backend, never by account, and `SOSStore.init` had no identity check before adopting a
> restored draft. Traced to its worst case: after the round-2 fix stopped clearing the outbox on an
> automatic 401 (see Task 12's section), a *second* account logging in on the same device would
> inherit the first account's draft and `resumeIfNeeded` would send it under the new token. If that
> draft had already reached the server, the second account reads the first account's case
> (read-only, still bad). If it had not — the exact case when the 401 that triggered the automatic
> logout *is the response to the SOS raise itself* — the server INSERTs a brand-new case owned by
> the second account's `participant_id`, seeded with the first account's position and message. A
> real, dispatchable emergency attributed to the wrong person, created by nothing more than logging
> in. See the post-review note at the end of Task 12's section for the fix, and Task 14's report for
> the full trace.

> **Third post-review addition (from Task 14's fourth review round), to both `SOSModels.swift` and
> `SOSOutbox.swift`:** the round-3 `ownerId` field above had no default and no custom decoder, so it
> relied on Swift's synthesized `Decodable`, which treats every non-optional stored property as a
> required JSON key. Any `SOSDraft` written to disk by a build predating this field has no
> `"ownerId"` key at all — decoding it threw, `SOSOutbox.current()`'s `try?` swallowed that into
> `nil`, indistinguishable from "nothing stored," and `SOSStore.init`'s two branches (`if let
> restored, ...` / `else if restored != nil`) both require a non-nil `restored` to do anything, so
> neither fired. Net effect: **a genuinely in-flight, unresolved case already on a device is
> forgotten the instant this build lands** — no crash, no banner, and the undecodable bytes are
> never cleared either, sitting on disk until some unrelated future `save()` happens to overwrite the
> key. Exactly the failure this file's own top comment exists to prevent
> ("เคส SOS ที่หายไปคือคนที่รออยู่บนดอยโดยเชื่อว่าส่งไปแล้ว"), reintroduced by a schema change with no
> migration path.
>
> Fixed with two changes, neither of which resurrects the old case (there is no way to know whose it
> was) — both make the loss a *handled*, tested outcome rather than an invisible one:
>
> 1. `SOSDraft` gained a hand-written `init(from: Decoder)` (and, since providing one turns off
>    Swift's synthesized memberwise initializer too, a hand-written memberwise `init` matching every
>    existing call site's shape). The decoder uses `decodeIfPresent(String.self, forKey: .ownerId) ??
>    ""` instead of a required decode — a missing key becomes an empty string rather than a thrown
>    error. An empty owner can never equal a real `currentUserId`, so it falls straight into the
>    existing "owner doesn't match" branch in `SOSStore.init` and gets cleared through the path round
>    3 already built — **not** defaulted to the current session's id, which would silently re-open the
>    exact cross-account adoption round 3 closed.
> 2. `SOSOutbox.current()` now distinguishes "no data at this key" (returns `nil`, nothing to do) from
>    "data present but undecodable" (clears the key immediately, then returns `nil`) by checking for
>    the data's existence before attempting to decode, rather than collapsing both into the same
>    `try?` result. With part 1 in place this only fires for genuine corruption, not a missing
>    `ownerId`.
>
> Four new tests — two in `SOSOutboxTests.swift` (`testALegacyDraftMissingOwnerIdDecodesWithAn
> EmptyOwnerInsteadOfFailing`, `testUnreadableBytesAreClearedByCurrentRatherThanLeftLingering`) and
> two end-to-end in `SOSStoreTests.swift` (`testALegacyDraftMissingOwnerIdIsNotAdoptedAndTheSlotIs
> Cleared`, `testCorruptedBytesAreNotAdoptedAndTheSlotIsCleared`) — all verified RED against the
> pre-fix code via the same scoped-`git stash` technique as prior rounds. Worth recording: the first
> RED run showed only the two `SOSOutboxTests` failing, not the `SOSStoreTests` pair — checking
> `SOSOutbox().current() == nil` after constructing the store proved nothing, since that call returns
> `nil` identically whether the slot was properly cleared or the same undecodable bytes are still
> sitting there failing to decode on every call. Fixed by asserting against
> `UserDefaults.standard.data(forKey:)` directly in the `SOSStoreTests` pair instead, which does
> distinguish the two — confirmed genuinely RED, then GREEN. See Task 14's report, round-4 fix
...
Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/SOSTransportTests 2>&1 | tail -20`
...
Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/SOSTransportTests 2>&1 | tail -20`
Expected: `** TEST SUCCEEDED **`, twelve tests (the brief's original five, `testStatusBecomesClosedWhenResolved` for the `.closed` branch, and six added after a Task 10 review caught `activeSOS` mishandling the server's real `null\n` response and four decode calls leaking a raw `DecodingError` instead of `AppError.retryable`)
...
- Produces: `SOSFix{lat, lng, accuracyM}`, `SOSLocator` (`@MainActor final class`) with `authorization: CLAuthorizationStatus`, `requestPermission()`, `cachedFix(maxAge:) -> SOSFix?`, `oneShot(timeout:) async -> SOSFix?`. Injectable through `SOSLocationProviding` so tests never touch real hardware. Also `static let shared`
  (added by Task 13's post-review fix, not in this task's original scope — see the note after the code
  block below): a retained singleton, because `SOSLocator()` constructed and used without being held
  anywhere lets ARC deallocate it — and the `CLLocationManager` inside it — before the OS finishes
  presenting or delivering the authorization result.
...
Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/SOSLocatorTests 2>&1 | tail -20`
...
/// ต้องเงียบ ไม่ใช่ crash "resumed multiple times" จึงต้องมีล็อกจริง ไม่ใช่แค่ธง Bool
private final class OneShotResume: @unchecked Sendable {
    ...
    func callAsFunction(_ fix: SOSFix?) {
        ...
        continuation = nil
        lock.unlock()
        cont?.resume(returning: fix)   // resume นอกล็อกเสมอ
    }
}
...
final class SOSLocator {
    ...
    ///
    /// **ห้ามใช้ withTaskGroup** — group รอ child ทุกตัวจบก่อนคืนค่าเสมอ ต่อให้ cancelAll()
    /// ไปแล้วก็ตาม และ checked continuation ที่รอ callback ของ provider ไม่ตอบสนอง cancel เอง
    /// ถ้า timeout ชนะ การรอ fix ที่ไม่มีวันมาจะค้าง oneShot ไว้ตลอดกาล — ยืนยันจากการรันจริง
    /// (SWIFT TASK CONTINUATION MISUSE, xcodebuild นิ่ง 0% CPU 6:44 นาที) ไม่ใช่ทฤษฎี
    /// async let ก็พังทางเดียวกัน · ให้สองฝั่งแข่ง resume continuation เดียวกันแทน
...
> **Post-review addition (from Task 13, not part of this task's original delivery):** the code above,
> as originally written, has no retained singleton — every caller was expected to hold its own
> `SOSLocator` instance (`SOSStore` does; it stores one as a property for the lifetime of an active SOS
> case). Task 13 added `SOSLocator().requestPermission()` as a one-off, unretained call right after
> login, which a reviewer caught: an unretained `SOSLocator` (and the `CLLocationManager` inside it) can
> be deallocated by ARC before the OS finishes presenting or delivering the authorization prompt — the
> exact "SOS ที่ไม่มีพิกัด โดยไม่มี error ให้เห็น" failure this whole task exists to prevent. Fix: added
> `static let shared = SOSLocator()` to the class (same shape as `PushManager.shared`), and Task 13's
> call site now reads `SOSLocator.shared.requestPermission()`. `SOSStore` keeps constructing and holding
> its own separate instance — different lifecycle, different reason, not consolidated onto `.shared`.
> See Task 13's report, "Fix report" section, for the full finding and verification.

- [ ] **Step 4: Run test to verify it passes**

Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/SOSLocatorTests 2>&1 | tail -20`
Expected: `** TEST SUCCEEDED **`, five tests (six after Task 13's post-review addition above)
...
- Produces: `@MainActor final class SOSStore: ObservableObject` with `@Published private(set) var draft: SOSDraft?`, `@Published private(set) var serverCase: SOSCase?`, `@Published private(set) var status: SOSStatus?`, `@Published private(set) var showCallFallback: Bool`, and methods `raise(forOther:token:)`, `attachNote(_:token:)`, `cancel(token:) async -> APIClient.SOSCancelOutcome?`, `flush(token:)`, `clearForLogout()`. Status polling starts automatically after the first successful send (private `startStatusPoll`, not a public method — there is no `pollStatus(token:)`; an earlier draft of this line named one that never existed in the code or tests below). Injectable `raiseCall` / `cancelCall` / `activeCall` closures following the `FeedbackStore.submitCall` precedent, plus `callFallbackDelay: Duration` for tests. Also produces `Config.emergencyPhoneDefault: String`, `.emergencyPhone: String`, `.cacheEmergencyPhone(_:)` — moved forward from Task 13 because `send(token:)` calls `cacheEmergencyPhone` directly, so `SOSStore.swift` cannot compile without it existing first. Task 13 consumes these rather than redeclaring them. (Post-review, from Task 14: also produces `resumeIfNeeded(token:) async`, `handleLogout(automatic:)`, and a `pollInterval: Duration` constructor parameter — see the note at the end of this task's section.)
...
final class SOSStoreTests: XCTestCase {
    ...
    func testASuccessfulSendMovesTheStatusToReceivedAndKeepsTheCase() async {
        ...
        // (พบตอนทำ Task 12 — ค่าเริ่มต้นที่ไม่ได้ฉีดไว้ของ activeCall/cancelCall ชี้ไป
        // APIClient.shared จริงเสมอ ต้องฉีดทุกตัวที่มีโอกาสถูกเรียกในเทส ไม่ใช่แค่ตัวที่บรีฟฉีดไว้แล้ว)
        // ฉีดเคสที่ resolved แล้วให้ poll loop จบตัวเองตั้งแต่รอบแรก ไม่ค้างเป็น background task ลอย
    ...
    /// resolved มีดีฟอลต์เป็น false เพื่อให้ call site เดิม sampleCase(acked:) ไม่ต้องแก้ —
    /// เพิ่มพารามิเตอร์นี้เข้ามาเพื่อฉีดเป็นค่าตอบของ activeCall stub ด้านบนเท่านั้น
...
> **พบตอนทำ Task 12 (ก่อน commit ใดๆ ทั้งสิ้น — ไม่ใช่จากการรันจริงทีหลัง):** เทสสองตัวข้างบน
> (`testASuccessfulSendMovesTheStatusToReceivedAndKeepsTheCase`,
> `testCancelAfterAckReportsAlreadyAckedAndKeepsTheCaseVisible`) เดิมไม่ได้ฉีด `activeCall`ไว้
> เลย ทั้งที่ `raiseCall` ของทั้งคู่คืนค่าสำเร็จ ซึ่งทำให้ `send()` เรียก `startStatusPoll` จริง
> ค่าเริ่มต้นของ `activeCall` คือ `APIClient.shared.activeSOS` ของจริง — ยูนิตเทสสองตัวนี้จะยิง
> long-poll ออกไปที่ `Config.apiBase` (ตอนเขียนคือ `.susProd` ของจริง) ซ้ำไปเรื่อยๆ เป็น background
> task ที่ไม่มีวันเลิกจนกว่าโปรเซสเทสจะปิด ไม่ใช่แค่ครั้งเดียว วิธีแก้คือฉีดเคสที่ `resolved: true`
> ให้ poll loop จบตัวเองตั้งแต่รอบแรก ตามที่ปรับไว้ด้านบน — ไม่มี call site ไหนนอกไฟล์เทสนี้เรียก
> `SOSStore(activeCall:)` เอง จึงไม่กระทบอะไรนอกไฟล์นี้

- [ ] **Step 2: Run test to verify it fails**

Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/SOSStoreTests 2>&1 | tail -20`
...
final class SOSStore: ObservableObject {
    ...
    // Task 12 ก่อน commit ใดๆ ทั้งสิ้น) ย้ายการสร้างเข้าไปใน body ของ init ซึ่งอยู่ใน MainActor
    // context จริงๆ แล้วแก้ปัญหานี้ได้ ไม่กระทบผู้เรียกที่ไหนเลย — ไม่มี call site ไหนในแผนทั้งฉบับ
    // ส่ง locator: เข้ามาเอง
    //
    // callFallbackDelay ย้ายมาเป็นพารามิเตอร์สุดท้าย (ดราฟต์ก่อนหน้าวางไว้ก่อน raiseCall) เพราะ
    // Swift บังคับให้ argument label เรียงตามลำดับที่ประกาศไว้เท่านั้น เรียงสลับที่ call site ไม่ได้ —
    // เทส testTheCallFallbackAppearsOnlyAfterTheCaseHasBeenStuck ด้านบนส่ง raiseCall: ก่อน
    // callFallbackDelay: ("argument 'callFallbackDelay' must precede argument 'raiseCall'" ตอน
    // คอมไพล์จริง — พบตอนทำ Task 12 เช่นกัน) ย้ายมาไว้ท้ายสุดแทนที่จะแก้เทส เพราะทุกเทสในไฟล์นี้
    // (รวมถึงเทสที่ไม่ได้แตะพารามิเตอร์นี้เลย) ส่ง raiseCall: มาก่อนเสมอ
...
> **Important finding from review, fixed here (post-implementation, before this plan doc was
> considered final):** `startLocationChase` spawned a bare `Task { … }` with no stored handle, so
> `finish()` could not stop it; separately, `send(token:)` captured the draft before its only
> suspension point and never re-checked state after resuming. Combined: a `cancel()` (local-only
> branch) or `clearForLogout()` racing against a `send()` suspended mid-`raiseCall` that later
> succeeds would resurrect the case the user just cleared — worst for logout, whose entire purpose is
> preventing exactly that inheritance across accounts. Fixed with a `generation` counter (bump on
> every clear, capture at the top of `send`, bail after the `await` if it no longer matches) plus
> giving `startLocationChase` a stored, cancellable `chaseTask` with its own post-await
> `Task.isCancelled` check, mirroring `fallbackTask`. Two tests added to Step 1 above (see
> `testClearForLogoutDuringASuspendedSendDoesNotResurrectTheCase` and
> `testCancelDuringASuspendedSendDoesNotResurrectTheCase`) using a `Gate` actor to force the clear to
> land while `raiseCall` is genuinely suspended — verified RED against the pre-fix code (reproduced
> the exact resurrection, `serverId` and all) before confirming GREEN with the fix.

> **Post-review additions (from Task 14, not part of this task's original delivery)** — four, all to
> `SOSStore.swift`:
>
> 1. `func resumeIfNeeded(token: String) async` — `init()` restores `draft`/`status` from the outbox
>    on a cold launch but deliberately does not restart `retryTask`/`pollTask`/`fallbackTask`/
>    `chaseTask` itself (a side-effecting initializer that silently fires network calls is hard to
>    reason about). Mirrors the tail of `raise()` — `startFallbackTimer()`, `send(token:)`,
>    `startRetryLoop`, `startLocationChase` — without building a new draft. `MainTabView`'s mount
>    `.task` calls it.
> 2. `func handleLogout(automatic: Bool)` — the single decision point `MainTabView.onDisappear` now
>    calls on every logout, participant or not. `Session`'s `.wbwUnauthorized` observer calls
>    `logout()` unconditionally on any 401 from anywhere in the app, with **no user confirmation at
>    all** — a straight `sos.clearForLogout()` there (what Task 14 originally shipped) would silently
>    strand someone with an open case at the login screen mid-emergency, violating this task's own
>    doc comment on `clearForLogout()` ("the caller must confirm first, not clear silently") and the
>    design spec's "logout with an open case: warn and require confirmation." `handleLogout(automatic:
>    true)` is now a no-op — the on-disk draft survives, and `resumeIfNeeded` picks it back up once
>    the same person re-authenticates. `handleLogout(automatic: false)` (the path from the
>    `SettingsView` "ออกจากระบบใช่หรือไม่" button, which already confirms) still calls
>    `clearForLogout()` exactly as before. Known, accepted residual risk *at the time*: `SOSOutbox` is
>    namespaced only by backend, not by user, so a *different* account logging in on the same device
>    before the original owner re-authenticates could inherit the draft — judged acceptable because
>    an automatic 401 is, in practice, overwhelmingly the same person's token expiring, not a device
>    handoff. **A third review round traced this further and found it was not acceptable — see the
>    round-3 note below, immediately after this one.**
> 3. `pollInterval: Duration = .seconds(1)` — new trailing constructor parameter (same pattern as
>    `callFallbackDelay`), purely for injecting a short interval in the test below; production
>    behavior unchanged.
> 4. `startStatusPoll` itself: (a) the `c.resolved` branch now also calls `outbox.clear()` and sets
>    `draft = nil` — previously only `finish()` ran, so a resolved case's `draft` (with its
>    `serverId`) survived on disk forever; a force-quit and relaunch after that point would have
>    `init()` re-derive `status = .queued` for a case that had already ended, since nothing about
>    `SOSDraft` records closure. `serverCase`/`status` are left at `.closed` so the status view still
>    shows the ending. (b) a bounded exit — `maxConsecutiveEmptyPolls = 20` — so a case the server no
>    longer recognizes at all (`activeCall` returning `nil` every time, which **at the time of this
>    round** also covered whatever `try?` swallows from a thrown error, since `activeSOS` doesn't
>    distinguish the two) cannot poll forever; the counter resets on any non-nil response. **A third
>    review round found that conflating "confirmed no case" with "the request itself failed" was
>    exactly backwards for this feature — see the round-3 note below.**
>
> Two new tests added to Step 1's file (`testAutomaticLogoutLeavesTheOutboxIntactForReAuthenticationTo
> Resume`, `testUserInitiatedLogoutStillClearsTheOutbox`) plus two more
> (`testAPollThatObservesResolutionClearsTheOutboxButKeepsClosedStatusVisible`,
> `testStatusPollGivesUpAfterRepeatedlyFindingNoCase`) — all four verified RED against the pre-fix
> code (via a scoped `git stash` of just the implementation files, keeping the tests) before
> confirming GREEN with the fix. See Task 14's report, fix section, for the full finding.

> **Post-review additions, round three (from Task 14's third review), all to `SOSStore.swift`
> unless noted:**
>
> 1. **Ownership.** `init(currentUserId: String = "", ...)` — new first parameter, stored as
>    `private let currentUserId`. On construction, a restored draft is only adopted
>    (`self.draft`/`self.status = .queued`) if `restored.ownerId == currentUserId`; otherwise the
>    outbox is cleared and nothing is adopted. `raise()` stamps `ownerId: currentUserId` into every
>    new `SOSDraft` (see the `SOSDraft.ownerId` post-review note in Task 9's section for why — this
>    closes the residual risk item 2 above admitted and turned out to be real). `resumeIfNeeded`
>    re-checks `draft.ownerId == currentUserId` as a second, defense-in-depth layer, even though the
>    file's structure means a mismatched draft can never reach that point once `init()` gates it.
>    `MainTabView` can't read `session.user?.userId` at `@StateObject` construction time
>    (`@EnvironmentObject` isn't injected yet at that point in SwiftUI's lifecycle), so it reads the
>    same identity straight off disk via the new `Session.currentUserIdFromDisk()` (see Task 13's
>    section) instead.
> 2. **Poll error/empty separation.** `startStatusPoll`'s `try? await self.activeCall(...)` became a
>    real `do`/`catch`. Only a *successful* `nil` (confirmed "no case") increments
>    `consecutiveEmpty` toward `maxConsecutiveEmptyPolls`; a thrown error resets nothing and instead
>    sleeps for a step of the new `errorBackoffSchedule: [Duration]` constructor parameter (default
>    `[1s, 2s, 5s, 10s, 30s]`, capped at the last element), mirroring `startRetryLoop`'s existing
>    schedule-array pattern. `APIClient` never sets `waitsForConnectivity`, so a real dead zone fails
>    requests almost immediately rather than after a long timeout — at the old 1 s poll interval with
>    no separation, twenty consecutive failures (the whole ceiling) could burn through in well under
>    a minute, the opposite of what the round-2 comment on `maxConsecutiveEmptyPolls` claimed it
>    tolerated.
> 3. **Visible, recoverable give-up.** New `@Published private(set) var statusCheckStopped`, set
>    `true` when the ceiling is genuinely reached (confirmed-empty responses only) and cleared at the
>    top of every fresh `startStatusPoll` call, and also from `finish()` itself (covers the
>    cancel/logout/resolved paths, which don't call `startStatusPoll` again) so a stale "stopped"
>    banner can't survive a case that's no longer relevant. New `func retryStatusCheck(token:)` —
>    a no-op unless `draft?.serverId != nil && status?.isActive == true`, otherwise restarts
>    `startStatusPoll`. `WBW/SOS/SOSStatusView.swift` (Task 14's file) gained a banner + "เช็คสถานะ
>    อีกครั้ง" button bound to `store.statusCheckStopped` / `store.retryStatusCheck(token:)`.
>
> Five new tests: `testADifferentOwnerNeverAdoptsTheStoredDraft`,
> `testRepeatedNetworkErrorsDoNotTripTheCeiling`, `testRetryStatusCheckRestartsPollingAfterItStopped`
> in `SOSStoreTests.swift`, plus the existing `testResumeIfNeededContinuesAQueuedCaseFoundAtInit` and
> `testStatusPollGivesUpAfterRepeatedlyFindingNoCase` were both extended (explicit matching owner id
> on the former; an added `statusCheckStopped` assertion on the latter) rather than duplicated. All
> new/changed assertions verified RED against the pre-fix code (same scoped-`git stash` technique as
> round two) before GREEN. `SOSOutboxTests.swift` and `SOSTransportTests.swift` needed one-line
> updates each (`ownerId:` added to existing `SOSDraft(...)` construction sites) purely to keep
> compiling — not new behavior under test. See Task 14's report, round-3 fix section, for the full
> finding.

In `WBW/Config.swift`, inside `enum Config`:

```swift
// ต้นฉบับของสามตัวนี้คือ Task 13 (ดู task-13-brief.md ขั้นที่ 5: plist สิทธิ์ตำแหน่ง + เบอร์กลางงาน)
// แต่ต้องยกมานิยามที่นี่ก่อน เพราะ SOSStore.send (ด้านบน) เรียก cacheEmergencyPhone(_:) ทันทีที่
// ยิงสำเร็จครั้งแรก — SOSStore.swift จึงคอมไพล์ไม่ผ่านถ้าไม่มีฟังก์ชันนี้อยู่ก่อน (พบตอนทำ Task 12 —
// เหมือนกับ SOSStaffCase ที่ Task 10 เจอกับ Task 15 มาก่อนแล้ว) Task 13 consume ของสามตัวนี้แทนที่
// จะประกาศซ้ำ
...
Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/SOSStoreTests 2>&1 | tail -20`
Expected: `** TEST SUCCEEDED **`, seven tests originally — nine after the two race tests added post-review (see the note above the `Config.swift` block)

- [ ] **Step 5: Commit**

```bash
cd ~/wbw-ios-fontend
git add WBW/SOS/SOSStore.swift WBWTests/SOSStoreTests.swift WBW/Config.swift
...
**Files:**
- Modify: `WBW/Info.plist`, `WBW/Info-Debug.plist`, `WBW/PrivacyInfo.xcprivacy`, `WBWTests/AppStoreConfigTests.swift`, `WBW/Session.swift` (where login actually succeeds — the only call site of `session.save(`; see Task 13's report for why this was chosen over `LoginView.swift` itself)
- Test: `WBWTests/AppStoreConfigTests.swift`

**Interfaces:**
- Consumes: `SOSLocator.requestPermission()` (Task 11); `Config.emergencyPhoneDefault` / `.emergencyPhone` / `.cacheEmergencyPhone(_:)` (Task 12 — moved there because `SOSStore.send` calls `cacheEmergencyPhone` directly, so `SOSStore.swift` cannot compile without it existing first. See Task 12's note. This task consumes them rather than redeclaring them; `WBW/Config.swift` is therefore not touched here).
- Produces: nothing new beyond the plist keys, privacy manifest entry, and the post-login permission request below — no new Swift symbols.
...
Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/AppStoreConfigTests 2>&1 | tail -20`
...
At the point where login succeeds, request location permission. This is the same *timing* move the
paused push brainstorm already approved for the notification prompt — ask after login, not at launch —
but it does not mean building a combined permission screen: this task's own Interfaces line above scopes
it to "no new Swift symbols," so implement this as a plain call at the existing post-login choke point,
not a new screen.
...
(No `await`: the natural call site is `Session.save(_:)`, which is itself synchronous and already
`@MainActor` — same as `SOSLocator` — so the call never crosses an actor boundary, and `await` would not
compile inside a non-`async` function. `PushManager.shared.registerCurrent()` already lives in
`save(_:)` for the identical "right after login succeeds" reason, and `save(_:)` is the only place
`session.save(` is ever called from — so it was chosen over editing `LoginView.signIn()` directly.)

**`.shared`, not `SOSLocator()` (post-review fix, not the first-pass delivery):** the first pass wrote
this as a bare `SOSLocator().requestPermission()` with nothing retaining the instance. A reviewer caught
that ARC can deallocate an unretained `SOSLocator` — and the `CLLocationManager` inside it — before the
OS finishes presenting or delivering the authorization result, which is silent: no error, no test catches
it, the prompt just may never appear. Fixed by adding `static let shared` to `SOSLocator` itself (Task
11's file — see the post-review note in that task's section) and calling through it here instead. Also
narrowed the `#if DEBUG` `uitestToken` hook's comment in `Session.swift`, which had claimed to mirror
`save()` "in every way" — no longer true now that `save()` requests location and the hook deliberately
doesn't. Full finding and verification in Task 13's report, "Fix report" section.

- [ ] **Step 5: Add the emergency number to Config — already done, nothing to do here**

`Config.emergencyPhoneDefault`, `.emergencyPhone`, and `.cacheEmergencyPhone(_:)` were moved forward into
Task 12's `WBW/Config.swift` block (same field names, same bodies, byte-for-byte) because `SOSStore.send`
calls `cacheEmergencyPhone` directly and `SOSStore.swift` would not compile without it existing first —
see the note at the end of Task 12's Step 3. Confirm the three declarations are already present in
`WBW/Config.swift` before starting this task; there is nothing left to add here. (Same move as
`SOSStaffCase`, pulled from this plan's own Task 10/15 pair.)

`053-916-000` is a placeholder — replace it with the real number once operations provides it (still true,
just done a task earlier than originally planned) — see the prerequisites section of the spec.

- [ ] **Step 6: Run the full iOS suite**

Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' 2>&1 | tail -20`
Expected: `** TEST SUCCEEDED **`. `AppStoreConfigTests` now has 9 tests, not 8 — the file already had 6 before
this step (not 5), one of them (`testNoUsageDescriptionForFrameworksTheAppDoesNotUse`) is trimmed in
place to drop only its location assertion (its photo-library assertion stays), and Step 1 adds 3 new
ones on top: 6 + 3 = 9. The parity test still passes because the key was added to **both** plists.
(Total suite: 167 at this step's first pass. The post-review `.shared` fix above adds one more test to
`SOSLocatorTests` — see Task 11's post-review note — bringing the total to 168.)

- [ ] **Step 7: Commit**

```bash
cd ~/wbw-ios-fontend
git add WBW/Info.plist WBW/Info-Debug.plist WBW/PrivacyInfo.xcprivacy WBW/Session.swift WBWTests/AppStoreConfigTests.swift
git commit -m "feat(sos): เปิดสิทธิ์ตำแหน่งกลับมา — คีย์ plist, privacy manifest, ขอหลังล็อกอิน"
```

(`WBW/Session.swift`, not `WBW/LoginView.swift` — see the Files line at the top of this task's section.
The post-review `.shared` fix above is a separate commit, `fix(sos): ...`, touching
`WBW/SOS/SOSLocator.swift`, `WBW/Session.swift`, `WBWTests/SOSLocatorTests.swift`, and this plan doc
together — see Task 13's report.)

> **Post-review addition (from Task 14, not part of this task's original delivery):** `logout()`
> gained a parameter, `func logout(automatic: Bool = false)`, and a new published property,
> `@Published private(set) var lastLogoutWasAutomatic`. The `.wbwUnauthorized` observer this task's
> Step 5 wires up now calls `self.logout(automatic: true)` instead of `self.logout()`; both existing
> manual call sites (`SettingsView`'s confirmed "ออกจากระบบ" button, `StaffScanView`'s) are untouched
> and keep the default `false`. `MainTabView.onDisappear` reads `session.lastLogoutWasAutomatic` and
> passes it straight to `SOSStore.handleLogout(automatic:)` — see the post-review note at the end of
> Task 12's section for why an unconditional clear on this path was wrong. See Task 14's report for
> the full finding.

> **Second post-review addition (from Task 14's third review round):** `private let userKey =
> "wbw.user"` became `private static let userKey` (zero call-site changes needed inside instance
> methods beyond qualifying with `Self.` where Swift requires it for static member access from
> instance context), and a new `static func currentUserIdFromDisk() -> String` reads that same key
> directly from `UserDefaults`, decodes an `AuthUser`, and returns its `userId` (or `""` if never
> logged in / decode fails). `MainTabView` calls this from its own `init()` to construct `SOSStore`
> with the right `currentUserId` — see the `SOSStore.init` post-review note in Task 12's section for
> why this can't just read `session.user?.userId` as a `@StateObject` default value. Safe because
> `save()`/`logout()` always write the in-memory `@Published var user` and this UserDefaults key
> together, in the same function, before returning — never one without the other.
...
(Actual, after two rounds of review — see the note at the end of this section: also modified
`WBW/SOS/SOSModels.swift`, `WBW/SOS/SOSStore.swift`, `WBW/Session.swift`, `WBWTests/SOSStoreTests.swift`.
The re-entrancy guard this task is responsible for and the relaunch-resume gap flagged in Task 12's
review both turned out to need small, justified additions to files outside the original list, plus a
round-2 finding neither the brief nor the first review anticipated.)

**Interfaces:**
- Consumes: `SOSStore` (Task 12), `Config.emergencyPhone` (Task 13).
- Produces: `SOSHoldProgress` (the pure timing model: `start()`, `release()`, `progress: Double`, `didComplete: Bool`), `SOSButton`, `SOSStatusView`. Also produces (post-review) `SOSStatus.isActive`, `SOSStore.resumeIfNeeded(token:)`, `SOSStore.handleLogout(automatic:)`, `SOSStore`'s `pollInterval` parameter, and `Session.logout(automatic:)` / `.lastLogoutWasAutomatic` — see the individual post-review notes in Tasks 9, 12, and 13's sections, and the summary note at the end of this section.
...
Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/SOSButtonTests 2>&1 | tail -20`
...
> **What actually shipped, across four review rounds — see `task-14-report.md` for all four.**
>
> **Round 1** (this task's own two carried-in concerns, both explicitly out of Task 12's scope and
> handed to whoever built this task): `SOSButton` checks `store.status?.isActive` before calling
> `raise()`, both on press-down and again on hold-completion, plus a `firedThisHold` latch that makes
> "fire at most once per hold" hold synchronously rather than depending on SwiftUI's render timing
> (found during this task's own self-review, not carried in). `SOSStatusView` also gained a
> "ปิดหน้านี้" exit for the `.closed` case (the brief's given code had none) and used
> `SOSLocator.shared` instead of allocating a `CLLocationManager` per body evaluation. `MainTabView`
> gained `SOSStore.resumeIfNeeded(token:)` wired into its mount `.task`. Files beyond the four above:
> `WBW/SOS/SOSModels.swift` (`SOSStatus.isActive`), `WBW/SOS/SOSStore.swift`
> (`resumeIfNeeded(token:)`), `WBWTests/SOSStoreTests.swift` (+2 tests). Commit `d391485`. Suite: 175
> (168 baseline + 7).
>
> **Round 2** (coordinator review found two more, both explicitly assigned to this task rather than
> deferred): an automatic 401 logout was clearing an active case with no confirmation
> (`Session.logout(automatic:)` / `.lastLogoutWasAutomatic`, `SOSStore.handleLogout(automatic:)` —
> see the post-review notes in Task 12 and Task 13's sections), and `startStatusPoll` could poll
> forever for a case the server had forgotten, because nothing cleared the on-disk draft when a case
> resolved (`SOSStore`'s `startStatusPoll` fix + `pollInterval` parameter — see the post-review note
> at the end of Task 12's section). Files beyond round 1: `WBW/Session.swift`. Additional tests: +4 in
> `WBWTests/SOSStoreTests.swift`, all verified RED against the pre-fix code via a scoped `git stash`
> of just the three implementation files (tests kept in place) before confirming GREEN. Suite: 179
> (175 + 4).
>
> **Round 3** (coordinator review found two more, both again assigned to this task): round 2's own
> fix for the automatic-logout problem left the surviving draft ownerless, and a second account
> logging in on the same device could inherit — and in the no-`serverId`-yet case, actually
> fabricate — a case attributed to the wrong person (`SOSDraft.ownerId`, `SOSStore.init`'s ownership
> gate, `Session.currentUserIdFromDisk()` — see the post-review notes in Task 9, 12, and 13's
> sections); and the poll ceiling from round 2 counted network errors the same as confirmed-empty
> responses, so it could trip on an ordinary signal gap in well under a minute with no visible
> indication or way back (`startStatusPoll`'s `do`/`catch` split, `errorBackoffSchedule`,
> `statusCheckStopped`, `retryStatusCheck(token:)`, and a new banner + button in
> `SOSStatusView.swift` — see the same Task 12 post-review note). Files beyond round 2:
> `WBW/SOS/SOSModels.swift` (`SOSDraft.ownerId`), `WBW/SOS/SOSStatusView.swift`. Additional tests: +3
> new in `WBWTests/SOSStoreTests.swift`, plus 2 existing ones extended rather than duplicated, plus
> one-line `ownerId:` fixes (not new behavior) in `WBWTests/SOSOutboxTests.swift` and
> `WBWTests/SOSTransportTests.swift`. All new/changed assertions verified RED against the pre-fix
> code (same scoped-`git stash` technique) before GREEN. Suite: 182 (179 + 3).
>
> **Round 4** (coordinator review found one more, again assigned to this task): round 3's `ownerId`
> field had no default and no custom decoder, so any `SOSDraft` written to disk by a build predating
> that field failed to decode entirely — `SOSOutbox.current()`'s `try?` turned that into `nil`,
> indistinguishable from "nothing stored," so a genuinely in-flight, unresolved case already on a
> device was silently forgotten the moment this build landed, with the undecodable bytes never
> cleared either. Fixed with a hand-written `SOSDraft.init(from:)` that decodes a missing `ownerId`
> as `""` instead of throwing (plus the memberwise `init` that writing a custom decoder costs you),
> and `SOSOutbox.current()` now clears the slot itself when data is present but undecodable, rather
> than returning the same `nil` for that and for "nothing stored" — see the post-review note in Task
> 9's section (it owns both `SOSModels.swift` and `SOSOutbox.swift`). Files: `WBW/SOS/SOSModels.swift`,
> `WBW/SOS/SOSOutbox.swift`. Additional tests: +2 in `WBWTests/SOSOutboxTests.swift`, +2 end-to-end in
> `WBWTests/SOSStoreTests.swift` — all four verified RED against the pre-fix code (same scoped-`git
> stash` technique). Worth recording: the first RED run showed only two of the four failing — the
> `SOSStoreTests` pair's original assertion (`SOSOutbox().current() == nil`) passed even against the
> buggy code, since that call returns `nil` identically whether the slot was actually cleared or the
> same undecodable bytes are still sitting there. Strengthened to assert
> `UserDefaults.standard.data(forKey:) == nil` directly, which does distinguish the two, before
> reconfirming RED then GREEN. Suite: 186 (182 + 4).
>
> The staff-side SOS button the design spec calls for ("staff can press SOS too") was confirmed to be
> in neither this task nor Task 15's brief as written, and was reassigned to Task 15 rather than
> built here.
...
> **Scope addition from Task 14's round-2 review, not in the original brief below:** the design spec
> says "staff can press SOS too — the same floating button is mounted in the staff UI" (a staff
> member can fall down a slope like anyone else). `RootView.swift` routes every `staff`/`admin` login
> to `StaffScanView`, never `MainTabView`, so Task 14's `SOSButton` mount (participant-only) never
> reaches staff. Confirmed this was in neither Task 14's brief nor this task's as originally written —
> reassigned here rather than left to fall through the cracks. `SOSButton`/`SOSHoldProgress` can be
> reused as-is; this needs its own `SOSStore` instance (staff have no `participant_profile` row, so
> `group_id` on a staff-raised case is `NULL` — see the spec) and a `showStatus` flag on whichever
> staff screen mounts it.
...
**Interfaces:**
- Consumes: `APIClient.staffSOSFeed` / `ackSOS` / `resolveSOS`, `SOSStaffCase` (Task 10 — moved there because `staffSOSFeed`'s return type needed it to exist before this task runs; see Task 10's note).
- Produces: `StaffSOSStore` (`@MainActor`, long-poll loop, `cases: [SOSStaffCase]`, `ack(id:)`, `resolve(id:reason:)`), `StaffSOSView`.
...
Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/StaffSOSTests 2>&1 | tail -20`
...
> **Post-review addition, round one — what shipped differs from the snippet above in four ways, all
> found during self-review before the coordinator's pass:**
>
> 1. **`StaffHomeView`, not `staffHome`-on-`RootView`.** The snippet above implies `sosStaff`/
>    `staffOwnSOS` live directly on `RootView`. That's wrong: `RootView` is the single app-lifetime
>    root and never unmounts across login/logout, so a `@StateObject`'s `currentUserId` (read once
>    from disk in `init()`) would freeze at whatever was cached at the *first* app launch and never
>    update for later staff logins in the same process — reopening the exact cross-account
>    draft-leak class of bug `SOSDraft.ownerId` (Task 14, round 3) exists to close. Fixed by
>    extracting `private struct StaffHomeView: View` (in `RootView.swift`, mirroring how
>    `MainTabView.swift` carries `ForestBlank` alongside itself) mounted at the same `switch phase {
>    case .home: ... }` level `MainTabView()` already sits at, so it gets fresh identity — and a
>    fresh `init()` read of `currentUserId` — on every staff login.
> 2. **`host.suppressed` is not touched by tab selection.** The snippet's `.onChange(of: staffTab) {
>    host.suppressed = (tab != 0) }` conflates two unrelated systems: `host.suppressed` gates the
>    RealityKit forest scene, not `StaffScanView`'s `AVCaptureSession`. Writing it that way would
>    *un-suppress* the forest scene whenever staff return to the scan tab, contradicting the existing
>    invariant that staff never render the forest scene regardless of sub-screen. Left
>    `host.suppressed` exactly as the pre-existing `.onChange(of: isStaff)` had it; the camera's own
>    stop/start is `ScannerVC.viewWillDisappear`/`viewDidLoad`, unmodified, triggered by ordinary
>    `TabView` tab-switch lifecycle.
> 3. **`StaffSOSAlertView` invented** — referenced by the snippet's `.fullScreenCover(item:)` but
>    absent from this task's "Produces" list. Wraps `StaffSOSCard` with two dismiss controls (X
>    button, "ดูรายการเคสทั้งหมด"), both calling `dismiss()`, which only clears `newCase` —
>    `store.cases` is untouched by dismissal, so the case stays visible in the list afterward.
>    `.id(c.id)` added to the content closure: if a second new case supersedes `newCase` while the
>    first's cover is still open, SwiftUI may reuse the view instance rather than re-present it, and
>    `StaffSOSCard` carries `@State private var showReasons` that could leak across cases without it
>    — same class of bug `MainTabView.sheet(item: $feedbackCheckpoint)` already guards with
>    `.id(target.id)`.
> 4. **`StaffSOSStore.newCase` is new** (not in the "Produces" list) — the actual signal
>    `.fullScreenCover(item:)` binds to. Tracks a `seenIDs` set to distinguish a genuinely new case id
>    from an update to a known one, and treats the *first* `apply()` call as a baseline (existing
>    backlog on login shouldn't stack full-screen covers) rather than alerting on it. `cursor` changed
>    from fully `private` to `private(set)` and `pollInterval: Duration` added as an injectable
>    constructor parameter (default `.seconds(1)`, matching the original) purely so the cursor
>    round-trip could be tested without a real per-tick wait.
>
> Files: `WBW/SOS/StaffSOSView.swift`, `WBW/RootView.swift`. Tests: 12 in
> `WBWTests/StaffSOSTests.swift` (the 5 above plus 7 covering cursor pairing/round-trip and
> `newCase` detection). Suite: 198 (186 + 12).
>
> **Post-review addition, round two (one Critical, one Important):**
>
> - **Critical — the alert never fired for the first real case of the day.** `apply()`'s baseline
>   check used `seenIDs.isEmpty`, but `seenIDs` is populated only from non-empty `incoming`; an
>   emergency feed is empty for most of an event, so leading empty long-poll responses left `seenIDs`
>   empty indefinitely. The first genuinely new case to arrive after any quiet stretch was still read
>   as "baseline" and silently dropped — reproduced with `apply([])`, `apply([])`, `apply([case])`
>   leaving `newCase` nil. Fixed by tracking `hasPolledBefore` as its own flag, set unconditionally at
>   the end of every `apply()` call regardless of whether that call carried any cases, and keying the
>   baseline decision on that instead of `seenIDs`.
> - **Important — a staff member's own case could fight their own status screen.** `sosStaff` (the
>   "everyone else" feed) had no way to exclude the logged-in staff member's own case, so pressing the
>   staff member's own `SOSButton` could make the same case arrive through the feed and trigger
>   `StaffSOSAlertView` ("มีเหตุฉุกเฉินใหม่", no cancel action) at roughly the same moment
>   `staffOwnSOS`'s `SOSStatusView` (which does have a cancel action) wants to present — two
>   independent `.fullScreenCover`s racing with no guaranteed resolution order. Fixed by adding
>   `currentUserId: String` to `StaffSOSStore`'s initializer and excluding
>   `participantId == currentUserId` from the alert candidates only — the case still appears in the
>   list normally. `StaffHomeView.init()` reads `Session.currentUserIdFromDisk()` once and passes the
>   same value to both `sosStaff` and `staffOwnSOS`.
>
> Camera-stop (relies on `ScannerVC.viewWillDisappear` firing through a `UIViewControllerRepresentable`
> inside a `TabView`, a different mechanism from the `ForestSceneHost` onAppear/onDisappear-ordering
> proof cited for it in round one above) was judged plausible but unverified by the reviewer and
> carried to the on-device verification list rather than fixed in code — no cheap assertion was
> available (Simulator has no camera; `StaffScanView.swift` is outside this task's file scope).
>
> Files: same two, plus 4 more tests in `WBWTests/StaffSOSTests.swift` (empty-polls-before-first-case,
> own-case-excluded, other's-case-still-included, own-case-still-in-list). Suite: 202 (198 + 4).
...
Run: `cd ~/wbw-ios-fontend && xcodebuild test -scheme WBW -destination 'platform=iOS Simulator,name=iPhone 17' -only-testing:WBWTests/SOSNotificationRoutingTests 2>&1 | tail -20`
...
> **Post-review addition — what shipped differs from the snippet above in two ways:**
>
> 1. **The three-entry-point routing itself is not in the brief's code** — only named in prose
>    ("route into it from three places, matching how `FeedbackView` is already reached"). Built by
>    reading how `checkin_feedback`'s existing entry points actually work, not by inventing a new
>    shape: `NotificationsView` gained `onOpenSOS: (Int64) -> Void` (mirrors `onOpenFeedback`
>    exactly, including `NotiCard` growing an `isSOS` case — red accent reusing the existing
>    "emergency" color, the `"sos"` SF Symbol `StaffSOSAlertView` already uses, and the tap-chevron,
>    so an SOS card can't look tappable-but-isn't or the reverse). `MainTabView` gained
>    `sosFriendTarget: SOSFriendTarget?` as the single convergence variable (same shape as
>    `feedbackCheckpoint`), presented via `.sheet(item:)` with the same `.id(target.id)` fix
>    `FeedbackView`'s sheet already carries (a second case arriving while the first's sheet is open
>    must get fresh `@State`, not the first case's stale `sosCase`/`loadError`), plus
>    `pendingSOSFromNoti` (the same two-sheets-cannot-both-present deferred-open pattern
>    `pendingFeedbackFromNoti` already uses) and `pendingReadSOSId`/`markSOSNotiRead` (mirrors
>    `pendingReadCheckpoint`/`markFeedbackNotiRead` — without it, opening a case via push tap would
>    leave its badge stuck unread). `AppDelegate` gained a `case "sos":` branch in `didReceive`
>    (posts `.openSOSCase`, carries `sos_id` the same shape `checkpoint_id` does) and a branch in
>    `willPresent`, gated on `PendingPush.sosId(from: info) != nil` rather than a raw `type == "sos"`
>    check (a malformed SOS payload falls through to the system's default banner instead of being
>    silently swallowed) — suppresses the system banner and posts a new no-payload `.sosArrived`,
>    which only reloads the notification list rather than force-opening `SOSFriendView`, matching
>    `checkin_feedback`'s `willPresent` precedent exactly (a passively-arriving push is not
>    permission to interrupt whatever the user is doing). Reviewed and confirmed against that
>    precedent line for line.
>
> 2. **Important, found in review — "เปิดแชทกลุ่ม" did nothing visible.** The brief's button code
>    (`NotificationCenter.default.post(name: .openGroupChat, object: nil)`, nothing else) is correct
>    as far as it goes, but `SOSFriendView` is presented as a `.sheet` covering the same `ZStack`
>    that `GroupChatView` renders inside as a plain conditional, not a competing system presentation.
>    Posting alone sets `chatOpen = true`/`tab = 3` underneath the still-open sheet — invisibly, since
>    the sheet is drawn on top of the whole `ZStack`. `.openGroupChat` had exactly one other producer
>    (`AppDelegate.didReceive`, a push-tap context with no sheet up at the time), so there was no
>    working precedent this copied — the same "looks tappable but produces no visible effect" defect
>    class this task already reasoned carefully about elsewhere (the mirror image of the dead
>    Sign-Up/Forgot-Password text found on the login screen for App Review). Fixed with
>    `@Environment(\.dismiss)` in `SOSFriendView`, called immediately after the post — same pattern
>    `SOSStatusView`/`StaffSOSAlertView` already use for self-dismissal. Ordering matters and is
>    deliberate: `NotificationCenter` delivers to `.onReceive` synchronously with no scheduler in
>    between (the same assumption this file's own cold-launch `PendingPush` replay already depends
>    on), so `chatOpen` is `true` before `dismiss()` is even called, let alone before its animation
>    finishes — no frame where the sheet has closed onto an empty background. **No automated test
>    covers this**: `sosFriendTarget` is raw `@State` on `MainTabView` itself, same as
>    `feedbackCheckpoint`/`chatOpen`/`showNotifications` — none of this project's tests exercise
>    `MainTabView`'s own View-level state directly, because none of it is backed by a testable
>    store/router, and this project has no SwiftUI view-rendering or UI-tap harness (ViewInspector
>    would be a new SPM package, which is disallowed; this codebase's own comments note repeatedly
>    that no tap tooling exists here either). Verifying the chat actually becomes visible after the
>    dismiss animation is a by-eye simulator check, not yet performed — flagged for hardware
>    verification rather than faked with a test that would assert something adjacent instead of the
>    actual behavior in question.
>
> Also corrected `NotificationItem.refId`'s doc comment, which still said it was used only for
> `type == "checkin_feedback"` after `sosId` became its second reader.
>
> Files: same as Step 5, plus no new test files. Suite: still 206 (202 + this task's 4 — the fix
> commit adds no tests, for the reason above).
