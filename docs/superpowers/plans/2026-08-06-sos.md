...
- Produces: `SOSFix{lat, lng, accuracyM}`, `SOSLocator` (`@MainActor final class`) with `authorization: CLAuthorizationStatus`, `requestPermission()`, `cachedFix(maxAge:) -> SOSFix?`, `oneShot(timeout:) async -> SOSFix?`. Injectable through `SOSLocationProviding` so tests never touch real hardware.
...
```bash
cd ~/wbw-ios-fontend
git add WBW/Info.plist WBW/Info-Debug.plist WBW/PrivacyInfo.xcprivacy WBW/LoginView.swift WBWTests/AppStoreConfigTests.swift
